...
const UpdateEvent = () => {
  ...
  const [isAvailable, setIsAvailable] = useState(true);
    ...
    const handleChange = (e) => {
        if (e.target.name === "decor") {
            // Assuming decor initially is an object and has a category property
            setFormData({ ...formData, decor: { ...formData.decor, category: e.target.value } });
        } else {
    ...
    return (
        ...
        <form onSubmit={handleSubmit} className="grid grid-cols-1 gap-4">
            ...
            <div>
                ...
                <input type="text" id="decor" name="decor" value={formData.decor.category} onChange={handleChange} className="border rounded p-2 w-full"/>
            </div>
            <div>
